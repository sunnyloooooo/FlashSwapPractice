...
contract Arbitrage is IUniswapV2Callee, Ownable {
    ...
    function uniswapV2Call(address sender, uint256 amount0, uint256 amount1, bytes calldata data) external override {
        ...
        (uint112 reserve0, uint112 reserve1, ) = IUniswapV2Pair(callbackData.targetSwapPool).getReserves();
        uint256 amountOut = _getAmountOut(callbackData.borrowAmount, reserve0, reserve1);
        IUniswapV2Pair(callbackData.targetSwapPool).swap(0, amountOut, address(this), "");
    ...
    function arbitrage(address priceLowerPool, address priceHigherPool, uint256 borrowETH) external {
        ...
        CallbackData memory callbackData = CallbackData({
            ...
            borrowToken: IUniswapV2Pair(priceLowerPool).token0(),
            debtToken: IUniswapV2Pair(priceLowerPool).token1(),
            borrowAmount: borrowETH,
            debtAmount: borrowETH,
            debtAmountOut: _getAmountIn(borrowETH, reserve1, reserve0)
        });
